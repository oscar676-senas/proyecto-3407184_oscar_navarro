...
const entityData = {
  ...
  name: 'App de contabilidad para PYMEs',
  description: 'Centra, simplifica y automatiza la gestion financiera y administrativa de un negocio',
  identifier: 'codigo: abc-098', // código, id, número, etc.

  // Propiedad de contacto o ubicación (si aplica)
  contact: {
  email: 'oscar.sena006@gmail.com',
  phone: '0000000000',
  location: 'virtual'
  },
  ...
  items: [
   { name: 'facturacion', level: 95, category: 'ingresos' },
   { name: 'gestion de gastos', level: 90, category: 'gastos' },
   { name: 'impuestos', level: 85, category: 'fiscal' },
   { name: 'balances', level: 80, category: 'reportes' }
  ],

  // Array de enlaces o referencias (si aplica)
  links: [
   { platform: 'web', url: 'htpps://pymenob.com', icon: '✔' }
  ],
  ...
  stats: {
   ...
   active: 860,
   rating: 4.6,
   custom: 320  // renombra según tu dominio
  }
};
...
const renderBasicInfo = () => {
  ...
  const { name, description, contact: { email} } = entityData;

  // TODO: Actualiza los elementos del DOM usando template literals
  entityName.textContent = name;
  entityDescription.innerHTML = `<p>${'es una herramienta digital en la cual se puede utilizar para un buen manejo de la gestion de datos financieros y administrativos de un negocio y gracias a esto nos permite automatizarla y simplificarla por lo tanto dueños de negocios sin conocimientos contables avanzados pueden llevar el control de su dinero'}</p>`;
};
...
// ============================================
// TODO 5: Renderizar categorías de servicios
// ============================================
...
const renderStats = () => {
  ...
  // TODO: Crea array con labels descriptivos en español
   const statsArray = [
     ...
     { label: 'valoración media', value: stats.rating },
     { label: 'facturas', value: stats.custom }
   ];

  // TODO: Genera el HTML usando map()
   const statsHtml = statsArray.map(stat => `
     <div class="stat-pymes">
...
const toggleTheme = () => {
  // TODO: Implementa el cambio de tema
   ...
   const newTheme = currentTheme === 'batman' ? 'superman' : 'batman';
   document.documentElement.dataset.theme = newTheme;

  // TODO: Actualiza el ícono del botón
   themeToggle.textContent = newTheme === 'batman' ? '☀️' : '🌙';
...
const loadTheme = () => {
  // TODO: Carga el tema desde localStorage
   const savedTheme = localStorage.getItem('theme') ?? 'superman';
   document.documentElement.dataset.theme = savedTheme;
   themeToggle.textContent = savedTheme === 'batman' ? '☀️' : '🌙';
};
...
const copyInfo = () => {
  // TODO: Construye el texto a copiar
   ...
   const infoText = `
     ${name}
     ${'es una herramienta digital en la cual se puede utilizar para un buen manejo de la gestion de datos financieros y administrativos de un negocio y gracias a esto nos permite automatizarla y simplificarla por lo tanto dueños de negocios sin conocimientos contables avanzados pueden llevar el control de su dinero'}
